...
export const HorizontalProductCard = ({ product }) => {
    ...
    return (
        <div className="w-full max-w-[700px] flex flex-col sm:flex-row bg-white rounded-2xl shadow-lg overflow-hidden transition-transform duration-300 hover:scale-[1.005] hover:shadow-xl">
            {/* Product Image */}
            <img
                className="w-full sm:w-64 h-64 object-cover sm:rounded-l-2xl cursor-pointer"
                src={product.image}
                alt={product.title}
            ...
            <div className="flex flex-col justify-between p-5 w-full">
                ...
                {/* Price Section */}
                <div onClick={() => navigate(`/products/${product._id}`)} className="flex items-center gap-2 mb-3">
                ...
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <div className="flex items-center border rounded-md overflow-hidden">
                        <button
                            ...
                            className="px-3 py-1 text-lg font-bold text-slate-700 hover:bg-gray-100"
                        >
                            –
                        </button>
                        <span className="px-4 py-1 text-slate-900 font-medium">{product.quantity}</span>
                        <button
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    <button
                        className="flex items-center justify-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow-md transition-colors duration-200 hover:bg-slate-700 focus:ring-2 focus:ring-blue-400"
                        onClick={() => removeFromCart(product._id)}
                    >
                    ...
                    <button
                        onClick={() =>
                            !isInWishList && (addToWishList(product), removeFromCart(product._id))
                        }
                        className={`flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-medium shadow-md transition-colors duration-200 focus:ring-2 focus:ring-pink-400 ${isInWishList
